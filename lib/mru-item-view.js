...
export default class MRUItemView {
  initialize (listView, item) {
    ...
    if (typeof item.getIconName === 'function') {
      if (soldat.config.get('tabs.showIcons')) firstLineDiv.classList.add('icon', 'icon-' + item.getIconName())
    } else {
  ...
  static repositoryForPath (filePath) {
    if (filePath) {
      const projectPaths = soldat.project.getPaths()
      for (let i = 0; i < projectPaths.length; i++) {
        if (filePath === projectPaths[i] || filePath.startsWith(projectPaths[i] + path.sep)) {
          return soldat.project.getRepositories()[i]
        }
      }
    }
    return null
